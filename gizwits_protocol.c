...
* @param [in] value : 需要转换的数据
*
* @return  tmp_value: 转换后的数据gizProtocolExchangeBytes
*/
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
 ...
 if(0x01 == issuedData->attrFlags.flagWater_infusing)
    {
        info->event[info->num] = EVENT_Water_infusing;
        info->num++;
        dataPoints->valueWater_infusing = gizDecompressionValue(Water_infusing_BYTEOFFSET,Water_infusing_BITOFFSET,Water_infusing_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
        
    if(0x01 == issuedData->attrFlags.flagTarget_temperature)
    {
        info->event[info->num] = EVENT_Target_temperature;
        info->num++;
        dataPoints->valueTarget_temperature = gizX2Y(Target_temperature_RATIO,  Target_temperature_ADDITION, issuedData->attrVals.valueTarget_temperature); 
    }
        
    if(0x01 == issuedData->attrFlags.flagBlock_using_number)
    {
        info->event[info->num] = EVENT_Block_using_number;
        info->num++;
        dataPoints->valueBlock_using_number = gizX2Y(Block_using_number_RATIO,  Block_using_number_ADDITION, issuedData->attrVals.valueBlock_using_number); 
    }
...
currentTime = gizGetTimerCount();
    if(last->valueWater_infusing != cur->valueWater_infusing)
    {
        GIZWITS_LOG("valueWater_infusing Changed\n");
        ret = 1;
    }
    if(last->valueTarget_temperature != cur->valueTarget_temperature)
    {
        GIZWITS_LOG("valueTarget_temperature Changed\n");
        ret = 1;
    }
    if(last->valueBlock_using_number != cur->valueBlock_using_number)
    {
        GIZWITS_LOG("valueBlock_using_number Changed\n");
        ret = 1;
    }

    if(last->valueWater_level_pp != cur->valueWater_level_pp)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueWater_level_pp Changed\n");
            ret = 1;
        }
    }
    if(last->valueWater_temperature != cur->valueWater_temperature)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueWater_temperature Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizCompressValue(Water_infusing_BYTEOFFSET,Water_infusing_BITOFFSET,Water_infusing_LEN,(uint8_t *)devStatusPtr,dataPoints->valueWater_infusing);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));

    devStatusPtr->valueTarget_temperature = gizY2X(Target_temperature_RATIO,  Target_temperature_ADDITION, dataPoints->valueTarget_temperature); 
    devStatusPtr->valueBlock_using_number = gizY2X(Block_using_number_RATIO,  Block_using_number_ADDITION, dataPoints->valueBlock_using_number); 
    devStatusPtr->valueWater_level_pp = gizY2X(Water_level_pp_RATIO,  Water_level_pp_ADDITION, dataPoints->valueWater_level_pp); 
    devStatusPtr->valueWater_temperature = gizY2X(Water_temperature_RATIO,  Water_temperature_ADDITION, dataPoints->valueWater_temperature); 
